...
//Eliminar router mediante movimientos.
void Enrutador::eliminarVecino(const string& destino) {
    vecinos.erase(remove_if(vecinos.begin(), vecinos.end(), [&](const Enlace& e) { return e.getDestino() == destino; }), vecinos.end());
}
...
//generar grafica de tabla de costo.
